...
class ThirdPartyModel extends BaseModel
{
    ...
    public function giveCoupon($params)
    {
        ...
        $setting = $ShakeSettingModel->getOne($setting_where);
        $url = $setting['setting_value'] . '?access_token=' . $this->token;
        \Log4p::info(['type' => 'ThirdParty/giveCoupon', 'request' => ['url' =>$url], 'response' => ['msg' =>'发送优惠卷记录']]);
       $data = $this->getHttp($url, $givedata);
        \Log4p::info(['type' => 'ThirdParty/giveCoupon', 'request' => ['url' =>$url], 'response' => ['msg' =>$data['errmsg']]]);
//        /**测试**/
//        $data =['errcode'=>0,'msg'=>'成功'];
        $error['winning_id'] = $params['winning_id'];
        $error['shake_id'] = $params['shake_id'];
        $error['post_data'] = json_encode($givedata);
     ...
     * 补发
     */
    public function Reissue($winning_id,$shake_id,$url,$params){
        //重新拼接token
        $new_url = preg_replace('/(?:access_token=)(.*)/','access_token='.$this->token,$url);
        \Log4p::info(['type' => 'ThirdParty/Reissue', 'request' => ['url' =>$new_url], 'response' => ['file' =>__FILE__,'line'=>__LINE__]]);
       $data = $this->getHttp($new_url, $params);
        \Log4p::info(['type' => 'ThirdParty/Reissue', 'request' => ['url' =>$new_url], 'response' => [['file' =>__FILE__,'line'=>__LINE__,'msg'=>$data['errmsg']]]]);
//        /**测试**/
//        $data =['errcode'=>0,'msg'=>'成功'];
        $error['winning_id'] = $winning_id;
        $error['shake_id'] = $shake_id;
    ...
    public function giveRed($params)
    {
        ...
        $setting = $ShakeSettingModel->getOne($setting_where);
        $url = $setting['setting_value'] . '?access_token=' . $this->token;
        \Log4p::info(['type' => 'ThirdParty/giveRed', 'request' => ['url' =>$url], 'response' => ['msg' =>'1']]);
      $data = $this->getHttp($url, $giveData);
        \Log4p::info(['type' => 'ThirdParty/giveRed', 'request' => ['url' =>$url], 'response' => ['msg' =>$data['errmsg']]]);
//        /**测试**/
//        $data =['errcode'=>0,'msg'=>'成功'];
    ...
    public function getToken()
    {
        ...
        \Log4p::info(['type' => 'ThirdParty/token', 'request' => ['url' =>$url,'data'=>$params], 'response' => ['msg' =>'获取token','data'=>$data['data']['access_token']]]);
        //存入缓存
