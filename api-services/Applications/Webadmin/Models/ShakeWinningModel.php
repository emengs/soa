...
{
    private $files=['winning_id','shake_id','prize_id','play_record_id','channel_id','user_id','user_name','level','prize_type','prize_name','prize_send_status','prize_send_result','create_time','update_time'];
     ...
     * 清除中奖缓存
     */
    public function DelUserDrawdata($shake_id,$key){
        return  $this->RedisCache($this->Redis_prefix_name.$shake_id)->delUserDrawdata($shake_id,$key);
    }
     ...
     * 获取指定队列的元素
     */
    public function getDrawList($parmas){

        $start = $parmas['limit']*($parmas['page']-1);
        $end = $parmas['limit']*($parmas['page']);
        return $this->RedisCache($this->RdisQueuePrefix)->getQueueList($this->RedisDrawKeyName,$start,$end);
    }

    /**
     * 获取中奖详情
     */
    public function getDrawInfo($param){
        $shake_id=$param['shake_id'];
        $shop_id=$param['channel_id'];
        $uid=$param['uid'];
        $uuid=$param['uuid'];
    $rs=$this->RedisCache($this->Redis_prefix_name . $shake_id)->getUserDrawdata($shop_id . '_' . $uid, $uuid);
       return $rs;
    }
     ...
     * 获取队列长度
     */
    public function getDrawListLen(){
