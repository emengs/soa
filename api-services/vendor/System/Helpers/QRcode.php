...
class QRspec
{
    ...
    {//QRSPEC_VERSION_MAX
        ...
        {
            $qk = self::$capacity[$i][QRCAP_WORDS];
            $kk = self::$capacity[$i][$level];
            $words = $qk - $kk;
            if ($words >= $size)
                return $i;
        }
...
class QRcode
{
    ...
    //----------------------------------------------------------------------
    public static function base64Encode($text, $size = 3, $level = QR_ECLEVEL_L, $margin = 4)
    {
        $enc = QRencode::factory($level, $size, $margin);
        return $enc->encode64($text);
    }
}
//##########################################################################
...
class QRencode
{
    ...
    //----------------------------------------------------------------------
    public function encode64($intext)
    {
        $tab = $this->encode($intext);
        $maxSize = (int) (QR_PNG_MAXIMUM_SIZE / (count($tab) + 2 * $this->margin));
        $id = md5(uniqid(md5(microtime(true)), true)); //random ID number
        $file = "capt" . $id . ".png";
        $image = QRimage::image($tab, min(max(1, $this->size), $maxSize), $this->margin);
        ImagePng($image, $file);
        $basecode = base64_encode(file_get_contents($file));
        unlink($file);
        return $basecode;
    }
