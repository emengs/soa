...
class Connection
{
    ...
    private static $_instance = array(); //存储数据连接对象数组
    /**
     * 选择哪些列
     *
 ...
 /**
     ...
     * @return string
     */
    protected function buildREPLACE()
    {
        return 'REPLACE'
        . $this->buildFlags()
        . $this->buildInto()
    ...
    public static function getInstance($conn = array()) {
    	if (is_array($conn) && !empty($conn)) {
    		//用于区分哪个实例的键
    		$key = md5($conn['host'].$conn['port'].$conn['user'].$conn['password'].$conn['dbname'].$conn['charset']);
           
    	} else {
    		return false;
    	}
        
    	if(!isset(self::$_instance[$key]) || FALSE == (self::$_instance[$key] instanceof self)) {
    		self::$_instance[$key] = new self($conn);
    	}
    ...
    protected function connect()
    {
        ...
        $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);//抛出 exceptions异常
        $this->pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);//禁用本地预处理语句的模拟,使用主动prepare防止SQL注入
//        $this->pdo->setAttribute(PDO::ATTR_AUTOCOMMIT, 1); //自动提交，在事务中请关闭自动提交
//        $this->pdo->setAttribute(PDO::MYSQL_ATTR_USE_BUFFERED_QUERY, true);//使用缓冲查询,mysql可用
    }
